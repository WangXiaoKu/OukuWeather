...
public class ChooseAreaActivity extends Activity {
    ...
    private OukuWeatherDB oukuWeatherDB;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化adapter并将它设置为ListView的适配器
        adapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, dataList);
        listView.setAdapter(adapter);
        //获取OukuWeatherDB实例
        oukuWeatherDB = OukuWeatherDB.getInstance(this);
        //设置ListView的点击事件
    ...
    private void queryProvinces() {
        provinceList = oukuWeatherDB.loadProvinces();
    ...
    //查询省中所有的市，优先从数据库中查询，如果没有再去服务器上查询
    private void queryCities() {
        cityList = oukuWeatherDB.loadCities(selectedProvince.getId());
        if (cityList.size() > 0) {
            dataList.clear();
